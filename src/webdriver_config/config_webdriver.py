...
class ConfiguracionWebDriver:
    ...
    def configurar_obtencion_web_driver(self):
        ...
        if len(self.ruta_web_driver.strip()) == 0:
            self.log.error(constantes_config_webdriver.MSG_ERROR_PROP_INI_WEBDRIVER_SIN_CONFIGURAR)
            sys.exit()

        elif self.driver_por_configurar == constantes_config_webdriver.CHROME:
            driver_configurado = self.inicializar_webdriver_chrome()
            self.log.info(constantes_config_webdriver.MSG_INFO_INICIO_CONFIGURACION_DRIVER.format(
                constantes_config_webdriver.CHROME))

        elif self.driver_por_configurar == constantes_config_webdriver.FIREFOX:
            driver_configurado = self.inicializar_webdriver_firefox()
            self.log.info(constantes_config_webdriver.MSG_INFO_INICIO_CONFIGURACION_DRIVER.format(
                constantes_config_webdriver.FIREFOX))
            ...
            self.log.info(constantes_config_webdriver.MSG_INFO_INICIO_CONFIGURACION_DRIVER.format(
                constantes_config_webdriver.PHANTOMJS))

        else:
            self.log.error(constantes_config_webdriver.MSG_ERROR_CONFIGURACION_DRIVER)
            sys.exit()
